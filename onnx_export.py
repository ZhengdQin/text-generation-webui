    ...
    try:
        ...
        if shared.args.no_stream:
            with torch.no_grad():
                ...
                # dummy_input = torch.tensor([[    1, 13103,  4060,  5155,   322,  6089,    13,    13, 16492, 29901,
                #                 4996,  2656,   297,   274,  1817, 29973,    13, 29943, 19304,  1234, 29901]]).npu()
                # torch.onnx.export(shared.model, 
                #       dummy_input, 
                #       "./llama_onnx/llama.onnx", 
                ...
                # hugging face optimum exprot, can use with generate function
                from optimum.exporters import TasksManager
                from optimum.exporters.onnx import export
                from collections import OrderedDict
                onnx_path = Path("model.onnx")
                onnx_config_constructor = TasksManager.get_exporter_config_constructor("onnx", shared.model)
                onnx_config = onnx_config_constructor(shared.model.config, task="causal-lm")
                onnx_inputs, onnx_outputs = export(shared.model, onnx_config, onnx_path, onnx_config.DEFAULT_ONNX_OPSET, disable_dynamic_axes_fix=True)
                exit()
                for i in range(10):
                    start = time.time()
                    # shared.model.save_pretrained("local-pt-ckpt")
                    # shared.tokenizer.save_pretrained("local-pt-ckpt")
                    # from pathlib import Path
                    output = shared.model.generate(**generate_params)[0]
                    print(f"1 answer time: {time.time()-start}")
                if cuda:
                    output = output.npu()
...
#     def __init__(self, threadID, question='Common sense questions and answers\n\nQuestion: how to learn a language?\nFactual answer:', 
#                 max_new_tokens=200, 
#                 do_sample=True, 
#                 temperature=1.99, 
#                 top_p=0.18, 
